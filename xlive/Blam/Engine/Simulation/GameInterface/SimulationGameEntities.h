...
public:
	...
	virtual  int state_data_size();
	virtual  int creation_data_size();
	virtual  signed int update_flag_count();
	virtual  int initial_update_mask();
	virtual  char sub_A90039(int a1);
	virtual  char sub_A90043(int a1);
	virtual  char sub_A90048(int a1, int a2);
	virtual  int  sub_A9004D(int a1, int a2, int a3);
	virtual  void get_creation_relevance_string(int a1, int a2, int a3, int a4);
	virtual  int calculate_update_relevance(int a1, int a2, int* a3);
	virtual  void write_creation_to_packet(int creation_data_size, void* creation_data, void* telementary_data, bitstream* packet);
	virtual  bool read_creation_from_packet(int creation_data_size, void* creation_data, bitstream* packet);
	virtual  bool write_update_to_packet(bool unk, int update_mask, int* update_mask_written, int state_data_size, void* state_data, void* telementary_data, bitstream* packet, int required_leave_space_bits);
	virtual  bool read_update_from_packet(bool a1, int* out_update_mask, int state_data_size, void* state_data, bitstream* packet);
	virtual  bool sub_A90891(int a1, int a2, int a3);
	virtual  char sub_A908DB(int a1, int a2, int a3);
	virtual  unsigned int build_creation_data(s_simulation_game_entity* entity, int creation_data_size, void* out_creation_data);
	virtual  bool build_baseline_state_data(int creation_data_size, void* creation_data, int state_data_size, void* baseline_state_data);
	virtual  bool build_updated_state_data(s_simulation_game_entity* entity, int* update_mask, int state_data_size, void* state_data);
	virtual  unsigned int sub_A9026A(int a1);
	virtual  bool create_game_entity(s_simulation_game_entity* entity, int creation_data_size, void* creation_data, int mask, int state_data_size, void* state_data);
	virtual  bool update_game_entity(s_simulation_game_entity* entity, int update_flags, int state_data_size, void* state_data);
